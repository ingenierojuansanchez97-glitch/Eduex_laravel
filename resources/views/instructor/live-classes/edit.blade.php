        ...
        <div class="dashboard-layout">
            ...
            <div class="dashboard-main">
                ...
                <div class="dashboard-courses-section wow fadeInUp" data-wow-delay="0.1s" style="margin-top: 30px;">
                    <form class="settings-form" action="{{ route('instructor.live-classes.update', $liveClass->id) }}" method="POST" style="background: #fff; padding: 30px; border-radius: 8px; border: 1px solid #f1f5f9;">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mb-4">
                            ...
                            <select id="lesson_id" name="lesson_id" class="form-control @error('lesson_id') is-invalid @enderror" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 4px;">
                                ...
                                @foreach($lessons as $lesson)
                                    <option value="{{ $lesson->id }}" {{ old('lesson_id', $liveClass->lesson_id) == $lesson->id ? 'selected' : '' }}>{{ $lesson->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $liveClass->title) }}" placeholder="e.g. Q&A and Interactive Review Session" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 4px;">
                            @error('title')
                                <span class="text-danger" style="font-size: 13px;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea id="description" name="description" rows="4" class="form-control @error('description') is-invalid @enderror" placeholder="Provide details about what will be covered in this session..." style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 4px;">{{ old('description', $liveClass->description) }}</textarea>
                            @error('description')
                                <span class="text-danger" style="font-size: 13px;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="url" id="join_url" name="join_url" class="form-control @error('join_url') is-invalid @enderror" value="{{ old('join_url', $liveClass->join_url) }}" placeholder="e.g. https://zoom.us/j/... or https://meet.google.com/..." required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 4px;">
                        ...
                        <div style="display: flex; gap: 20px; flex-wrap: wrap;" class="mb-5">
                            <div class="form-group" style="flex: 1; min-width: 250px;">
                                ...
                                <input type="datetime-local" id="scheduled_at" name="scheduled_at" class="form-control @error('scheduled_at') is-invalid @enderror" value="{{ old('scheduled_at', $liveClass->scheduled_at ? $liveClass->scheduled_at->format('Y-m-d\TH:i') : '') }}" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 4px;">
                                @error('scheduled_at')
                                    <span class="text-danger" style="font-size: 13px;">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group" style="flex: 1; min-width: 250px;">
                                ...
                                <input type="number" id="duration_minutes" name="duration_minutes" min="1" max="1440" class="form-control @error('duration_minutes') is-invalid @enderror" value="{{ old('duration_minutes', $liveClass->duration_minutes ?: 60) }}" required placeholder="e.g. 60" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 4px;">
                        ...
                        <div class="form-actions" style="border-top: 1px solid #f1f5f9; padding-top: 20px; text-align: right;">
                            ...
                            <button type="submit" class="theme-btn style-2" style="padding: 10px 20px; border-radius: 4px;">Update Class</button>
                        </div>
                    </form>
                </div>
            </div>
